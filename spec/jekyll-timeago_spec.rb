...
describe Jekyll::Timeago do
  ...
  context 'Core' do
    ...
    it 'allow to change level of detail' do
      ...
      expect(timeago(sample_date.prev_day(500), sample_date, "depth" => 5)).to eql('1 year and 4 months ago')
    end

    it 'allow localization' do
      expect(timeago(sample_date.prev_day(100), sample_date, locale: :es)).to eql('hace 3 meses y 1 semana')
    end
  end
